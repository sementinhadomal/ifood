...
async function upsertLead(input = {}, req = null) {
    if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
        ...
        if (!initialRecord.session_id) return { ok: false, reason: 'missing_session_id' };

        const leads = loadLocalLeads();
        const existingIndex = leads.findIndex(l => l.session_id === initialRecord.session_id);
        const existing = existingIndex !== -1 ? leads[existingIndex] : null;
        ...
        if (existing) {
            leads[existingIndex] = { ...existing, ...record };
        } else {
            leads.push(record);
        }
...
async function updateLeadByPixTxid(txid, fields = {}) {
    if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
        ...
        let updatedCount = 0;
        leads.forEach(l => {
            if (l.pix_txid === cleanTxid) {
                Object.assign(l, fields, { updated_at: new Date().toISOString() });
                updatedCount++;
            }
        });

        if (updatedCount > 0) saveLocalLeads(leads);
...
async function getLeadByPixTxid(txid) {
    if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
        ...
        if (!cleanTxid) return { ok: false, reason: 'missing_txid' };
        const leads = loadLocalLeads();
        const lead = leads.find(l => l.pix_txid === cleanTxid);
        return { ok: true, data: lead || null };
    }
...
async function updateLeadBySessionId(sessionId, fields = {}) {
    if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
        ...
        if (!cleanSession) return { ok: false, reason: 'missing_session_id' };
        const leads = loadLocalLeads();
        let updatedCount = 0;
        leads.forEach(l => {
            if (l.session_id === cleanSession) {
                Object.assign(l, fields, { updated_at: new Date().toISOString() });
                updatedCount++;
            }
        });
...
async function getLeadBySessionId(sessionId) {
    if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
        const cleanSession = String(sessionId || '').trim();
        const leads = loadLocalLeads();
        const lead = leads.find(l => l.session_id === cleanSession);
        return { ok: true, data: lead || null };
    }
...
async function findLeadByIdentity(identity = {}) {
    if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
        ...
        const lead = leads.find(l => {
            if (cpf && l.cpf === cpf) return true;
            if (email && l.email === email) return true;
