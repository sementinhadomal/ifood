...
function normalizeAuthCandidates(config = {}) {
    ...
    // AtivusHub Basic Auth requires base64(api_key + ":") per their docs.
    const rawWithColon = raw
        ? Buffer.from(`${raw}:`).toString('base64')
        : '';

    const candidates = [
        // Preferred: stored apiKeyBase64 (should already include colon if set correctly)
        token ? `Basic ${token}` : '',
        // Fallback: raw key with colon suffix encoded to base64 on the fly
        rawWithColon ? `Basic ${rawWithColon}` : '',
        // Legacy fallbacks
        token,
        raw ? `Basic ${raw}` : '',
        raw
