...
function issueSessionCookie(req, res) {
    ...
    // Safari can be sensitive to SameSite=None without Secure, but Lax is usually safe.
    // We add explicit Path / and ensure Secure is present in production.
    const cookie = `${cookieName}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${SESSION_TTL_SEC}${secure ? '; Secure' : ''}`;
    res.setHeader('Set-Cookie', cookie);

    // Add security headers to help Safari handle the response correctly
