...
function verifyAdminCookie(req) {
    // 1. Check for Bearer Token (Secure Server-to-Server Sync)
    const authHeader = String(req.headers.authorization || '').trim();
    if (authHeader.startsWith('Bearer ')) {
        const tokenPassword = authHeader.replace(/^Bearer\s+/i, '');
        if (verifyAdminPassword(tokenPassword)) {
            return true;
        }
    }

    // 2. Check for traditional Cookie (Browser Admin Panel)
